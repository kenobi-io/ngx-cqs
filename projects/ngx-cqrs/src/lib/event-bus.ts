import { Type, Injectable, OnDestroy, Injector } from '@angular/core';
...
import { ISaga } from './interfaces/saga.type';
...
export class EventBus extends ObservableBus<IEvent>
  implements IEventBus, OnDestroy {
  private _publisher: IEventPublisher;
  private readonly subscriptions: Subscription[];

  constructor(
    private readonly commandBus: CommandBus,
    private readonly moduleRef: Injector,
  ) {
    super();
    this.subscriptions = [];
  ...
  registerSagas(types: Type<any>[] = []) {
    const sagas = types
      .map(target => {
        const metadata = Reflect.getOwnMetadata(SAGA_METADATA, target) || [];
  ...
  private reflectEventsNames(handler: EventHandlerType): FunctionConstructor[] {
    return Reflect.getOwnMetadata(EVENTS_HANDLER_METADATA, handler);
  }
